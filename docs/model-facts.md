...
Snapshot accessed 2026-08-31 for `hy4-preview` and `qwen3.8-flash`; `glm-5.3-flash` was re-verified 2026-09-11. DeepSeek lab metadata, official API docs, and capability parameters were verified 2026-09-11. The current Vancine model ID `deepseek-v4.1-flash`, live ratios, and openai endpoint were re-verified from production `GET https://vancine.com/api/pricing` at 2026-09-15T07:51:35Z. `deepseek-flash` is a retired former Vancine ID, not a current alias.

`cacheWrite` is `0` because those sources published cache write as n/a, not because a write rate was guessed.

`compat.supportsDeveloperRole` is `false` from the known Vancine OpenAI-compatible Chat Completions behavior. It is a provider fact, not a per-model guess. `supportsReasoningEffort` is left unset so Pi's URL/provider auto-detect remains, except for `deepseek-v4.1-flash`, where the Vancine catalog publishes `true`: the Chat Completions request contract defines `reasoning_effort` and `thinking`, and the DeepSeek adaptor passes an explicitly supplied field through untouched for ids without a `deepseek-v4-*` suffix.
...
| deepseek-v4.1-flash | DeepSeek V4.1 Flash | text, image | true | 1000000 | 384000 | 0.24 / 0.96 / 0.0048 / 0 | lab `models/deepseek/deepseek-v4.1-flash.toml` and DeepSeek official docs (accessed 2026-09-11); planned Vancine provider entry `providers/vancine/models/deepseek-v4.1-flash.toml` (local, not yet public Models.dev API); Vancine current ID `deepseek-v4.1-flash` and live `/api/pricing` ModelRatio 0.12, CompletionRatio 4, CacheRatio 0.02 re-verified 2026-09-15T07:51:35Z |
